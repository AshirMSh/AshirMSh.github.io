---
title: "Vocation"
showTableOfContents: true
showAuthor: false
cardView: true
groupByYear: true
layout: "list"
---

*Vocation* — from the Latin **vocare**, "to call." This is the engineering path I've followed so far: the roles, the problems worth solving, and the systems I've helped bring to life.

## The Journey

{{< timeline >}}

{{< timelineItem icon="code" header="Data Centers Control Systems Engineer" badge="Jul 2025 – Present" subheader="Schneider Electric — Data Center and Secure Power Business · Grenoble, France" md="true" >}}
- Architecting next‑generation **800V DC microgrid and DC UPS** solutions for future data centers, translating customer requirements into end‑to‑end system architectures and technical specifications — securing **€50M in R&D funding**.
- Designed and implemented **CI/CD pipelines** (GitHub Actions, Docker, RaaS) and a scalable verification and validation framework for **200K+ LOC** projects, cutting PLC code generation and validation time by up to **80%**.
- Led **Hardware‑in‑the‑Loop (HIL)** validation framework development on Speedgoat real‑time hardware — timing analysis, fault injection, and requirements‑based verification — while training cross‑functional teams on HIL methodology.
- Developed **AI‑driven load smoothing** algorithms for DC UPS systems, enabling ancillary services (FCR, FRR) and reducing peak demand by **18%**.
{{< /timelineItem >}}

{{< timelineItem icon="star" header="Energy Simulation and Management Engineer" badge="Nov 2024 – Jul 2025" subheader="Schneider Electric — AI Hub / Energy Simulation Team · Grenoble, France" md="true" >}}
- Engineered **Model Predictive Control (MPC)** algorithms in MATLAB/Simulink for bidirectional EV charging in microgrids — **85% faster simulation**, **99.8% accuracy** for real‑time use.
- Designed battery degradation models using weighted Ah‑throughput, extending battery lifespan by **2+ years** in Vehicle‑to‑Building scenarios.
- Collaborated with the Sustainability Research Institute on energy market modeling, quantifying **20–30% residential energy savings**.
{{< /timelineItem >}}

{{< timelineItem icon="lightbulb" header="Junior Energy Engineer" badge="Jul 2022 – Aug 2023" subheader="K State Energy Systems Center and Black & Veatch · Manhattan, United States" md="true" >}}
- Developed requalification tests for second‑life EV battery applications in microgrids — **UL1974 certified**, 3 pilot projects commissioned.
- Designed a two‑stage optimization‑based battery management system — **20% improvement** in energy efficiency.
- Built and deployed an ML‑based **Arc Fault Detection** model on STM32 — **98.5% accuracy**, under **2 ms** inference latency.
{{< /timelineItem >}}

{{< /timeline >}}

*Photos from each chapter of the journey are coming soon.*

## Education

**IMT Atlantique (École Des Mines)** · Nantes, France
M.S. – Energy Engineering · *Sept 2023 – Sept 2025* · CGPA 3.85/4.00 · France Excellence for Climate Change Scholarship

**National University of Sciences and Technology** · Islamabad, Pakistan
Bachelors in Mechanical Engineering · *Sept 2019 – June 2022* · CGPA 3.94/4.00 · Double Gold Medal

**Kansas State University** · Manhattan, United States
Control Systems – Global UGRAD Exchange · *Aug 2022 – Aug 2023* · CGPA 4.00/4.00 · Dean's List Recipient

## Core Skills

| Category | Skills |
|---|---|
| **Controls & Simulation** | Electrical & physical modelling, digital twin, SCADA, industrial networking, PLC codegen, SIL/HIL/MIL testing |
| **Energy Systems** | Power management, DC UPS, cooling systems, battery systems (BESS), microgrid design & optimization |
| **Programming & Tools** | Python, MATLAB/Simulink, C++, GitHub Actions, Docker, CI/CD pipelines, Machine Learning (OpenCV, PyTorch) |
| **Project Management** | Agile methodologies, JAMA/JIRA requirements tracking, market research, scenario modelling |
| **Languages** | English (C2, bilingual), French (B2, intermediate) |

## Certifications

- Data Centers Specialization
- GitHub Actions Specialization
- Stanford Machine Learning Certification

## Case Studies

A closer look at some of the systems and models built along the way:
